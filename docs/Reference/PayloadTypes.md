Below is the full contents of the `return_types.py` file, which specifies the different ways that Atena sends results whenever `tree.async_run()` yields an object. This is primarily intended for interacting with the frontend - giving the frontend a specific 'payload type' and ensuring that all objects sent conform to the same 'mapping'.
